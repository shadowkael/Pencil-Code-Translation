...
函数是编程中最重要的概念，因为他们让编程分解成更小的子程序 然而，函数有另一个重要的目的，它们允许程序员设置代码进行运行，控制时，正是当代码运行时。

学生不可能体会到第一次创建子程序的价值，因为他们的价值很小。因此，我们建议开始对函数当成代码来运行。

命名函数，函数调用和函数参数，可以介绍一下作为推广的理念，一个强大的功能。最终学生要学会运用“DRY”的原则，用函数来创建自己的命令，通过创建自己的命令来抽象普通代码序列。
...
1.函数允许代码重用。一旦定义了一个函数，它的代码可以在一个程序中多个地方使用，而不必再写一行代码 。

2.函数控制何时以及如何运行 。当代码被放在一个函数里，它是不会运行的，但后来，当调用函数时候是可以的。关键是计算机代码理解函数是理解代码定义在一个函数中，不会立即执行。当程序执行一个函数调用时，程序会运行代码。这意味着，函数可以多时间被用来重复使用相同的代码。
